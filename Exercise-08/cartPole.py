...
class Bucketed_Q_Table():
    def __init__(self, observation_space, action_space, num_of_buckets):
        ...
        self._table = np.zeros(
            ([num_of_buckets] * len(observation_space.high) + [action_space.n]))
    ...
    def _create_buckets(self, observation_space, num_of_buckets):
        ...
        for i in range(num_of_observations):
            ...
            bucks.append(np.linspace(
                observation_space.low[i], observation_space.high[i], num_of_buckets + 1))
        return bucks
...
if __name__ == '__main__':
    ...
    def eps(episode): return max(
        (NUM_OF_EPISODES - 2*episode) / NUM_OF_EPISODES, 0)

    env = gym.make("CartPole-v0")

    q_table = Bucketed_Q_Table(
        env.observation_space, env.action_space, NUM_OF_BUCKETS)
