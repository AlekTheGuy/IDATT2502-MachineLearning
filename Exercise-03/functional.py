...
class ConvolutionalNeuralNetworkModel:
    ...
    def __init__(self):
        ...
        # Shape: (out_channels, in_channels, image height, image width)
        self.W1 = (torch.randn(32, 1, 5, 5) * std).requires_grad_()
        self.b1 = (torch.randn(32) * std).requires_grad_()

        std = 1 / math.sqrt(32 * 14 * 14)
        # Shape: (in_channels * image_pixels_after_max_pool, classes)
        self.W2 = (torch.randn(32 * 14 * 14, 10) * std).requires_grad_()
